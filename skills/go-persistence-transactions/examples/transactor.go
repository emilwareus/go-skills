// Package examples contains transaction patterns for a small User
// aggregate with points and Discounts.
//
...
//     fallback pattern. Prefer UpdateFn first (see update_fn.go) and
//     reach for TransactionProvider only when more than one repository
//     genuinely has to commit together.
package examples
...
// runInTx is deliberately minimal: no context, no isolation options.
// Both can be threaded through later if the project needs them, but
// the small shape is part of what makes this safe - there is nothing to
// misuse. The contract:
//
//   - If fn returns nil, COMMIT and return its (nil) error.
//   - If fn returns an error, ROLLBACK; if rollback itself fails,
//     join both errors so the post-mortem sees both.
//
...
// but the helper stays minimal on purpose - surprises
...
//
// This is the fallback pattern. It is more fragile than the
// UpdateFn-only approach because every SELECT in every repository
// must remember to use FOR UPDATE for the row-lock guarantees to
// hold across the whole tx. Use it when you genuinely need
// cross-repository atomicity (e.g. audit log + domain write), not as
// a default.
...
// Handler shape. The handler depends on
